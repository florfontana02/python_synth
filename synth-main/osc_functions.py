...
import numpy as np
from synth.components.oscillators import WavetableOscillator
...
def osc_wavetable1(freq, amp, sample_rate):
    """
...
def osc_wavetable(freq, amp, sample_rate):
    # usa params.master_volume, params.adsr_attack, etc.
    osc = WavetableOscillator(sine_table, freq, amp*params.master_volume, sample_rate)
    env = ADSREnvelope(
        attack_duration  = params.adsr_attack,
        decay_duration   = params.adsr_decay,
        sustain_level    = params.adsr_sustain,
        release_duration = params.adsr_release,
        sample_rate      = sample_rate
    )
    return iter(
        Chain(
